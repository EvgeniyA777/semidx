...
trigger_files="$(
  printf '%s\n' "$changed_files" |
    grep -E '^(src/|test/|scripts/|contracts/|proto/|deps\.edn$|ARCHITECTURE_CONSTITUTION\.md$|SPEC\.md$|GLOSSARY\.md$|README\.md$|RULES\.md$|AGENTS\.md$|CLAUDE\.md$|docs/(runtime-api|mcp-api|roadmap-status)\.md$|plans/|adr/|docs/(adr|design|plans)/)' || true
)"

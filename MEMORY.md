...
- `plans/021` is in progress for persistent JVM runtime reuse. Stage 0 selected `runtime-http` as the first reuse profile. Stage 1 delivered `semidx.runtime.launcher`, a pure state/health/process/lock decision kernel plus focused tests. Stage 2 should add the runnable local command path that reuses runtime HTTP before spawning a new JVM.

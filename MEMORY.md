...
- Language adapter extraction has started: `semidx.runtime.languages.shared` now owns generic line/signature/token and tree-sitter CLI/config/CST helpers, while existing adapters and extracted lanes keep their private helper names as wrappers to preserve behavior during the staged split.

...
1. Start `plans/013` Stage 3 by drafting the Stage 3 scoping ADR that applies the relation-first fork from `ADR-034` to the interprocedural/dataflow v1 slice.
2. Implement Stage 3 only after that ADR defines the exact first facts, typed-relation schema shape, query/runtime exposure, and benchmark/replay acceptance gates.

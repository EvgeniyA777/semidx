...
- Stage 3 relation substrate is implemented: `semidx.runtime.relations` owns relation normalization, deterministic IDs, schema versioning, and forward/reverse indexes; `semidx.runtime.index` now attaches empty `:relations`, `:relation_forward_index`, and `:relation_reverse_index` maps to snapshots without changing existing caller/callee retrieval behavior.
...
1. Continue `plans/013` Stage 3 with the Clojure `dataflow/*` producer on top of the existing relation substrate.
2. Continue Stage 3 with the Python producer and bounded retrieval/impact projections only after relation substrate and Clojure facts have focused coverage.

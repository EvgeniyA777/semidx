...
- Language adapter extraction has moved the Clojure lane out of `semidx.runtime.adapters`: `semidx.runtime.languages.clojure/parse-file` now owns Clojure regex, clj-kondo, and optional tree-sitter parsing, while `adapters/parse-file` dispatches directly to that lane without a legacy `adapters/parse-clojure-file` facade. `semidx.runtime.languages.shared` owns generic line/signature/token and tree-sitter CLI/config/CST helpers, including shared `safe-line` behavior still used by current TypeScript/Java adapter paths.

...
- ADR-040 fixes the Stage 4 public relation-traversal contract: there is one
  ...
  delivered; the library/MCP handlers and forward-only PostgreSQL relation
  projection remain implementation work.
...
3. Stage 4.1 is delivered: ADR-040 plus the JSON/malli relation-traversal
   request/result contract and validated examples. Next is the forward-only
   PostgreSQL `semantic_index_relations` projection and batched frontier provider,
   followed by library + MCP exposure with parity to the pure kernel.

...
1. Relation identity/evidence split and validation hardening are delivered under ADR-039 (identity excludes mutable resolution/evidence; invalid facts surface as `:relation_diagnostics`). Next: implement the storage-independent bounded traversal kernel under `runtime.relations` (start nodes, direction, relation-type allow-list, resolved-only behavior, cycle handling, deterministic ordering, and `max_depth`/`max_nodes`/`max_paths` budgets, initial bounds depth<=4/200 nodes/50 paths).
2. Then add bounded relation-backed retrieval/impact projections over that kernel, reason-coded and low-weight, preserving existing caller/callee outputs.

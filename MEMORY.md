...
- ADR-036 selects the Stage 2 tree-sitter dependency strategy: keep regex parsing as the guaranteed default, keep tree-sitter as an optional accelerated parser mode, and remove the hard dependency on an externally installed `tree-sitter` executable by resolving acceleration through explicit parser options, environment configuration, and a repository-managed toolchain provisioned by the grammar bootstrap path, with ambient `PATH` only as a developer fallback.
...
1. Implement `plans/013` Stage 2 according to ADR-036: add repo-managed tree-sitter toolchain resolution in shared helpers/bootstrap while preserving regex defaults and explicit unavailable-acceleration diagnostics.

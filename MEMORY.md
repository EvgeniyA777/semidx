...
- Benchmark scope boundary: Stage 2 owns orchestration only. Arms execute through the `ArmRunner` protocol (`scripted-arm-runner` for tests, `process-arm-runner` as a JSON stdin/stdout bridge); a live evaluated-model agent process is still missing and blocks Stage 4 scoring together with the Stage 0 calibration pilot and threshold lock. Stage 3 (aggregator command) must also confirm the benchmark feedback payload survives the PostgreSQL `jsonb` round-trip.

...
- Benchmark scoring hardening (2026-08-28, review fixes): a freshness-required task now fails with `missing_snapshot_evidence` when the answer reports no `snapshot_id`, and a run that cannot supply `current_snapshot_id` for such a task is refused (`benchmark_missing_current_snapshot_for_freshness_task`) instead of being scored as ordinary retrieval; required facts/symbols match answer text on token boundaries, so a short fact such as `ids` is no longer satisfied by `forbids`; the paired A-vs-B comparison weights success per task (`success_weighting: "task_mean"`) exactly as it weights cost, with the unweighted `attempt_success_rate` and its Wilson interval kept alongside.
...
- Benchmark scope boundary: Stage 2 owns orchestration only. Arms execute through the `ArmRunner` protocol (`scripted-arm-runner` for tests, `process-arm-runner` as a JSON stdin/stdout bridge); a live evaluated-model agent process is still missing and blocks Stage 4 scoring together with the Stage 0 calibration pilot and threshold lock. The PostgreSQL `jsonb` round-trip of the benchmark payload is confirmed by a `SEMIDX_TEST_POSTGRES_URL`-gated test added in Stage 3.
...
1. Continue `plans/020` at the Stage 0 second sub-gate: run the calibration
   pilot (5-10 tasks, competent-baseline cost and success noise floor) and lock
   the final threshold before any scoring. In parallel, deliver the live
   evaluated-model arm runner behind the existing `ArmRunner` contract; the
   scripted runner never contacts a provider and cannot measure a baseline.
   Stage 4 stays blocked on both. The eligible price schedule
   `2026-08-03-eligible-v1` expires 2026-10-16 and the harness refuses runs on
   or after that date.

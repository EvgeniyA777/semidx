...
- Language adapter extraction has moved the Clojure, Java, and Python lanes out of `semidx.runtime.adapters`: `semidx.runtime.languages.clojure/parse-file` owns Clojure regex, clj-kondo, and optional tree-sitter parsing; `semidx.runtime.languages.java/parse-file` owns Java regex and optional tree-sitter parsing, including overload, constructor, static-import, superclass, and method-reference call extraction; `semidx.runtime.languages.python/parse-file` owns Python module/import/call extraction, class/method ownership, relative imports, test linkage, and nested-scope suppression. `adapters/parse-file` dispatches directly to those lane namespaces without legacy `adapters/parse-clojure-file`, `adapters/parse-java-file`, or `adapters/parse-python-file` facades. `semidx.runtime.languages.shared` owns generic line/signature/token and tree-sitter CLI/config/CST helpers, including shared `safe-line` behavior still used by current TypeScript adapter paths.

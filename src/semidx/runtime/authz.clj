...
(def ^:private policy-operations
  #{"policy_read" "policy_promote" "policy_retire"})

(defn- policy-operation? [op-name]
  (contains? policy-operations op-name))

(defn- operation-allowed? [rules op-name]
  (if (policy-operation? op-name)
    (let [allowed (->> (or (map-get rules :allowed_operations) [])
                       (map operation-name)
                       set)]
      (contains? allowed op-name))
    true))
      ...
      (try
        (cond
          ...
          (if-let [rules (tenant-rules policy tenant-id)]
            (cond
              (not (operation-allowed? rules op-name))
              {:allowed? false
               :code :forbidden
               :message (str "tenant " tenant-id
                             " is not allowed to perform " op-name)}

              (policy-operation? op-name)
              {:allowed? true}
              ...
              (let [request-root (canonical-path (or root_path "."))
                ...
                (cond
                  ...
                  {:allowed? false
                   :code :forbidden
                   :message (str "tenant " tenant-id
                                 " has no allowed_roots configured")}

                  (not (some #(path-prefix? request-root %) allowed-roots))
                  {:allowed? false
                   :code :forbidden
                   :message (str "tenant " tenant-id
                                 " is not allowed for root_path " request-root)}
                  ...
                  {:allowed? false
                   :code :forbidden
                   :message (str "tenant " tenant-id
                                 " has invalid allowed_path_prefixes configuration")}

                  (and has-path-rules? (not (seq paths)))
                  {:allowed? false
                   :code :forbidden
                   :message (str "tenant " tenant-id
                                 " requires explicit paths for " op-name)}

                  :else
                  (let [normalized-paths (if (seq paths)
                                           (mapv normalize-relative-path paths)
                                           [])
                        invalid-path? (some nil? normalized-paths)
                        denied-path? (and has-path-rules?
                                          (some (fn [path]
                                                  (not (some #(path-allowed? path %)
                                                             path-prefixes)))
                                                normalized-paths))]
                    (cond
                      invalid-path?
                      ...
                      {:allowed? false
                       :code :forbidden
                       :message (str "tenant " tenant-id
                                     " is not allowed for one or more requested paths")}
            ...
            {:allowed? false
             :code :forbidden
             :message (str "tenant " tenant-id
                           " is not configured in authz policy")}))
        (catch Exception e
          {:allowed? false
           :code :internal_error
           :message (or (.getMessage e)
                        "authz policy evaluation failed")})))))

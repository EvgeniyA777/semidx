...
| MCP dogfood | `zig build dogfood` | The MCP preview on a temporary copy of this repository's source units: the stdio habit loop (health, outline, repository map and its cursor pages, definition lookup, references, context and a depth-2 traversal, edit, refresh) and refresh failure injection against a fresh-index oracle. Never edits the repository. Not part of `zig build test`; `-Ddogfood-failure-points=<n>` sets how many allocations of the refresh are failed (default 32). |
| Habit loop gate | `zig build preview-gate` | The canonical local preview habit-loop gate ([specification](../mcp/habit_loop_gate.md)): everything `zig build dogfood` runs as its `repository-copy` profile, plus a `fixture` profile over a small temporary root that proves honest degradation. Named hard gates fail with `gate: FAIL [<profile>] <gate>`; timings and sizes are printed as observations and never fail it. Not part of `zig build test`. |
...
| Release gates | No single command, and no release automation. | A preview release candidate records the command set it ran in its release notes; `zig build preview-gate` is the habit-loop part of that set. |

...
| [013](013_java_supertype_guard_relaxation.md) | fixed | `coverage_gap` | [Plan 012 report](../reports/012_java_semantic_quality_without_query_regression_progress.md#supertype-guard-opportunity) | A type with supertypes resolved no simple name beyond its unit. [Plan 014](../plans/014_java_receiver_coverage.md) records the hierarchy and lifts the guard on a closed chain: 1,690 claims converted, both reason families to zero. |
| [014](014_java_instance_receiver_calls.md) | fixed | `coverage_gap` | [Plan 012 report](../reports/012_java_semantic_quality_without_query_regression_progress.md#the-addressable-receiver-subset) | [Plan 014](../plans/014_java_receiver_coverage.md) admits covered simple value receivers and `this.m()` under ADR 012: 3,294 calls converted, `receiver_bound` 55,565 → 0, and the remaining cases split into named non-scope families. |
| [017](017_plan_012_external_evidence_reproducibility.md) | open | `process_defect` | [Plan 012 review](../reports/012_java_semantic_quality_without_query_regression_progress.md#post-closure-review-2026-09-20) | Narrowed by [Plan 013 Stage 0](../reports/013_unresolved_mentions_from_the_callee_anchor_progress.md#stage-0-reproducible-baseline), which recorded a new baseline on a named clone and reproduced Plan 012's whole-graph families to the unit. One check remains: nobody has drawn the sample from the written selection rule alone. |
...
| --- | --- | --- | --- | --- |
| [019](019_designator_may_carry_source_expression.md) | fixed | `bug` | [ADR 010](../adr/010_designator_is_a_structured_name.md), [Plan 013](../plans/013_unresolved_mentions_from_the_callee_anchor.md) | A designator is a name and an optional qualifier, the written form is evidence, and default MCP output carries no expression text: all 118,471 unresolved Java call designators on the measured clone are names, where 99,103 were invocation text. |

...
| Aspect | Current behavior |
| --- | --- |
| Definitions (facts) | Named top-level `class` and `interface` declarations (roles `class` and `interface`) and the methods declared directly in them (role `method`). An interface method with no access modifier is recorded as `public`, as Java defines it. Java vocabulary stays in extension labels: `java.construct`, `java.package`, `java.return_type`, `java.supertype_names`, member and field name lists, and the shape a reader cannot see from its own source — `java.supertypes` (`none`/`declared`) on a covered top-level type, `java.access` (`public`/`protected`/`package_private`/`private`) and `java.static` (`true`/`false`) on a method ([ADR 009](../adr/009_java_static_calls.md), [ADR 011](../adr/011_java_hierarchy_from_indexed_source.md), [ADR 012](../adr/012_java_value_receiver_calls.md)). A definition carrying no such label is `unknown` to a reader, which declines rather than assuming the permissive case. |
| Relationships (facts) | `file DEFINES class/interface`, `class/interface DEFINES method`. `REFERENCES` from a class or interface for field types, method return types, and declared supertypes to the one current top-level class or interface that resolves in the declaring unit's visibility scope. A cross-unit target records a dependency on the unit read. `CALLS` for an unqualified invocation when the enclosing type declares exactly one method of that name, the call is not inside a class body declared in the method, and any declared supertype chain is closed in indexed source and declares no method of that name. `CALLS` for `this.m()` and for a covered value receiver when the receiver's declared simple type resolves, the target type declares exactly one accessible method of that name, and a closed target chain declares no method of that name ([ADR 012](../adr/012_java_value_receiver_calls.md)). `CALLS` for an invocation qualified by a simple class name — `StringUtils.isNotEmpty(value)` — only when no binding, on-demand static import, or inherited field can obscure the receiver name; the name resolves as a type inside the visibility scope; the target declares exactly one accessible `static` method; and any required enclosing or target chain is closed. The fact names the declared method selected under these rules; inherited targets, dispatch, hiding, and overload selection are not recorded. |
...
| Unresolved | Qualified type names; type parameters; names a member type, field, or supertype chain may claim; units without a package; ambiguous names; invocations qualified by `super`, a literal, a field access, a chained call, `new T()`, or `a.b.C.m()`; value receivers whose declared type is generic, array, wildcard, inferred, qualified, or not covered by the current binding list; uncovered binding introducers; every simple-name receiver in a unit with an on-demand static import; a target method whose `java.static` label is absent for a class-qualified call; a receiver name that reaches no current top-level covered type, or one outside the visibility scope, each keeping the type rule's own explanation; an open or name-declaring target chain; a target method that is overloaded, non-static for class-qualified calls, or outside the covered access — each with its own reason family. Each import form has its own reason: an on-demand import (`import a.b.*;`) names a scope, a static import names a member, and a single-type import no indexed unit declares says exactly that. A name the package or the imported package declares only outside the unit's visibility scope is unresolved and says so, which is a different answer from a name nothing declares. |
...
| Unsupported | Top-level enums, records, and annotation types; class or interface members other than methods (fields as definitions, constructors, nested types, initializers, interface constants); unnamed classes; bodies nested deeper than 64 levels. |
| Not recorded | Parameter and local-variable types except as receiver bindings for the covered value-receiver rule, constructor calls (`new X()`), field reads and writes, inherited target selection, method hiding, virtual dispatch, overload selection by argument type, static imports used as bare names, scoped receivers (`a.b.C.m()`), and build/classpath semantics. Declared supertypes are recorded as references, and the chain is walked only to rule out hidden members; no relationship records that a type inherits another type's members. |
...
| Known false negatives | Non-simple value receivers, `super.m()`, inherited method targets, generic/array/wildcard/inferred receiver types, overloads, qualified type names, member types, fields as definitions, constructors, enum/record/annotation declarations, interface constants, and cross-module references a build tool would permit. semidx does not read build descriptors, jars, class files, or modules, so classpath-only symbols stay unresolved ([follow-up 011](../followups/011_java_cross_module_visibility.md)). |
| Evidence | Measured against a repository semidx does not own: apache/dubbo at `df9c5e1`, 119 Maven modules, 4,050 Java source units. One repository is evidence, not a coverage claim. The boundary numbers come from [Plan 010](../reports/010_java_resolution_boundaries_progress.md); static class-qualified calls from [Plan 012](../reports/012_java_semantic_quality_without_query_regression_progress.md#stage-7-external-remeasurement-documentation-and-closure); hierarchy and value-receiver coverage from [Plan 014](../reports/014_java_receiver_coverage_progress.md). Plan 014 added 617 interface declarations and 2,405 interface methods as definitions, recorded declared supertypes, converted 1,690 guard-blocked claims, then converted 3,294 value-receiver calls. Across the plan, Java call facts went from 5,028 to 9,518 and reference facts from 1,740 to 3,920 on the external clone; ingestion remained within the Plan 011 work bounds. |

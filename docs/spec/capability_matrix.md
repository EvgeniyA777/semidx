...
| Bounds | Every list has a limit and reports its total and whether it was truncated; list tools report the applied limits in `budget`. `semidx_repo_map` and `semidx_context` default to a compact detail level that keeps each claim's resolution category, producer name, freshness, and evidence location, and `detail: "full"` renders every field. Responses as a whole are not size-bounded, and each result repeats its structured object as JSON text. |

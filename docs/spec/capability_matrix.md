...
| Bounds | Every list has a limit and reports its total and whether it was truncated; list tools report the applied limits in `budget`. `semidx_repo_map` and `semidx_context` default to a compact detail level. Compact context keeps each relationship's and focus existence claim's resolution category, producer name, freshness, and evidence location; `detail: "full"` context renders every field. The repository map is orientation at both levels: its definitions carry freshness and location but no existence resolution or producer, and `detail: "full"` restores its pre-detail fields only; `semidx_find_definitions` and `semidx_context` return existence provenance. Responses as a whole are not size-bounded, and each result repeats its structured object as JSON text. |
